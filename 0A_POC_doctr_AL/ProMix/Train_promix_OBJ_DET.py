...
def train(epoch, net, net2, optimizer, labeled_trainloader, pi1, pi2, pi1_unrel, pi2_unrel):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, true_labels, index) in enumerate(labeled_trainloader):
        ...
        ## MODIFIER debias_output POUR L'APLLIQUER A DES LOGITS DE SIGMOID: FAIRE DES RECHERCHES
        ...
        with torch.no_grad():
            ...
            ## MODIFIER PX ET PX2: CE DOIT ETRE DES PREDICTIONS SIGMOID 
            px = torch.softmax(outputs_x_ori, dim=1)
            px2 = torch.softmax(outputs_a_ori, dim=1)
            
            # debiasing for the generation of pseudo-labels
            ## MODIFIER debias_pl POUR QUE CE SOIT UN DEBIAS POUR SIGMOID: FAIRE DES RECHERCHES SUR LE CAS SIGMOID CAR LA CEST SOFTMAX
            ...
            # APPLIQUER LE THRESHOLD DE DECISION POUR GET PRED_NET
            ...
            # MODIFIER POUR QU'IL Y AIT AUSSI LES 0, PEUT ETRE APPLIQUER LA CONF ENTROPY
            ...
            if epoch > args.num_epochs - args.start_expand:
                # MODIFIER SCORE PAR LA CONF ENTROPY
                score1 = px.max(dim=1)[0]
                score2 = px2.max(dim=1)[0]
                # MODFIIER POUR DONNER POURCENTAGE DE SIMILARITÉ > TAUX (0.95)
