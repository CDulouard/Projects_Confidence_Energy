...
def train(epoch, net, net2, optimizer, labeled_trainloader):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, true_labels, index) in enumerate(labeled_trainloader):
        ...
        output_final = ((outputs_x.clone().detach().cpu()+outputs_a.clone().detach().cpu())/2).numpy().argmax(axis=1)
        total += batch_size
        with torch.no_grad():
            ...
            debias_px = outputs_x_ori
            debias_px2 = outputs_a_ori
            debias_px_unrel = outputs_x_ori
            debias_px2_unrel = outputs_a_ori
        ...
        targets_urel = ptx
        loss_unrel_ph = CEsoft(outputs_x_ph[idx_unchosen], targets=targets_urel[idx_unchosen]).mean()\
                  + w * CEsoft(outputs_x2_ph[idx_unchosen], targets=targets_urel[idx_unchosen]).mean()
        #loss for net1-pseudo head
        loss_net1_ph = beta * loss_unrel_ph + loss_ce_ph + w * (loss_cr_ph)
        ...
        loss_net2 = loss_ce2 + w * (loss_cr2)

        # unrel loss for reliable samples on the pseudo head 
        ptx2 = debias_px2_unrel  ** (1 / args.T)
        ptx2 = ptx2 / ptx2.sum(dim=1, keepdim=True)
        targets_urel2 = ptx2
        loss_unrel2_ph = CEsoft(outputs_a_ph[idx_unchosen_2], targets=targets_urel2[idx_unchosen_2]).mean()\
                  + w * CEsoft(outputs_a2_ph[idx_unchosen_2], targets=targets_urel2[idx_unchosen_2]).mean()
        #loss for net2-pseudo head
        loss_net2_ph = beta * loss_unrel2_ph + loss_ce2_ph + w * (loss_cr2_ph)

        # NegEntropy
        penalty = conf_penalty(outputs_x2) + conf_penalty(outputs_x2_ph)
        penalty2 = conf_penalty(outputs_a2) + conf_penalty(outputs_a2_ph)
        penalties = penalty +  penalty2
        # total loss
        loss = loss_net1 + loss_net2 + loss_net1_ph + loss_net2_ph + penalties + contrastive
...
def warmup(epoch, net, net2, optimizer, dataloader):
    ...
    for batch_idx, (inputs_w, labels, _) in enumerate(dataloader):
...
def eval_train(model, all_loss, rho, num_class):
    ...
    prediction_list = torch.zeros((50000,args.num_class))
    num_class = 0
    with torch.no_grad():
        for batch_idx, (inputs, targets, index) in tqdm(enumerate(eval_loader)):
            ...
            for b in range(inputs.size(0)):
                ...
                prediction_list[index[b]] = outputs_cpu[b]
    ...
    return prob, all_loss, prediction_list, targets_list

confidence_energy = lambda p, index: (np.mean((p+1e-5)*np.log(p+1e-5),axis=-1)+np.log(p[index]+1e-5))/2
...
loader = dataloader.cifarn_dataloader(args.dataset, noise_type=args.noise_type, noise_path=args.noise_path,
                                      is_human=args.is_human, batch_size=args.batch_size, num_workers=multiprocessing.cpu_count()-1, \
...
for epoch in tqdm(range(args.num_epochs + 1)):
    ...
    #    print('Warmup Net1 and Net2')
    #    warmup(epoch, dualnet.net1, dualnet.net2, optimizer, warmup_trainloader)
    if True:
        rho = args.rho_start + (args.rho_end - args.rho_start) * linear_rampup2(epoch, args.warmup_ep)
        prob1, all_loss[0], prediction_list1, targets_list = eval_train(dualnet.net1, all_loss[0], rho, args.num_class)
        prob2, all_loss[0], prediction_list2, targets_list = eval_train(dualnet.net2, all_loss[0], rho, args.num_class)
        #pred1 = Selecting_GMM(prediction_list1, prediction_list2, targets_list)
        pred1 = (prob1 > args.p_threshold)
        total_trainloader, noisy_labels = loader.run('train', pred1, prob1, prob2)  # co-divide
        train(epoch,dualnet.net1, dualnet.net2, optimizer, total_trainloader) 
