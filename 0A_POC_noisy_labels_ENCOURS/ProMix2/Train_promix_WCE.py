...
import argparse
import numpy as np
import dataloader_cifarn2 as dataloader
from model import *
...
def train(epoch, net, net2, optimizer, labeled_trainloader, pi1, pi2, pi1_unrel, pi2_unrel):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, true_labels, index) in enumerate(labeled_trainloader):
        ...
        if args.penalty:
            # penalties
            penalty = conf_penalty(outputs_x).mean() + conf_penalty(outputs_a).mean()
        else:
            penalty = 0
...
def warmup(epoch, net, net2, optimizer, dataloader):
    ...
    for batch_idx, (inputs_w, inputs_s, labels, _) in enumerate(dataloader):
        ...
        outputs_, outputs_embedding_ = net(inputs_s, contrastive=True)
        outputs2_, outputs_embedding2_ = net2(inputs_s, contrastive=True)
        l_ce = CEloss(outputs, labels)
        l_ce2 = CEloss(outputs2, labels)

        if args.contrastive:
            outputs_embedding_concatenated = torch.cat((outputs_embedding.unsqueeze(1), outputs_embedding_.unsqueeze(1)), dim=1) 
            outputs_embedding_concatenated_ = torch.cat((outputs_embedding2.unsqueeze(1), outputs_embedding2_.unsqueeze(1)), dim=1) 
            contrastive1 = ContrastiveLoss(outputs_embedding_concatenated)
            contrastive2 = ContrastiveLoss(outputs_embedding_concatenated_)
            contrastive = contrastive1 + contrastive2
        else:
            contrastive = 0

        if args.penalty:
            penalty = conf_penalty(outputs).mean() + conf_penalty(outputs2).mean()
        else:
            penalty = 0
...
def eval_train(epoch, model, model2, all_loss, rho, num_class):
    w = linear_rampup2(epoch, args.warmup_ep)
    model.eval()
    losses = np.zeros(50000)
    losses2 = np.zeros(50000)
    targets_list = np.zeros(50000)
    prediction_list = np.zeros(50000)
    outputs_list = np.zeros((50000,10))
    num_class = 0
    with torch.no_grad():
        for batch_idx, (inputs, targets, index) in enumerate(eval_loader):
            ...
            loss2 = CE(outputs2, targets) + conf_penalty(outputs2) 
            ...
            for b in range(inputs.size(0)):
                ...
                losses2[index[b]] = loss2[b]
                targets_list[index[b]] = targets_cpu[b]
                outputs_list[index[b]] = (outputs[b] + outputs2[b])/2


    #class-wise small-loss selection (CSS for base selection set)
    losses_ = (losses - losses.min()) / (losses.max() - losses.min())
    losses2_ = (losses2 - losses2.min()) / (losses2.max() - losses2.min())
    all_loss.append(losses_)
    all_loss.append(losses2_)
    input_loss = losses_.reshape(-1, 1)
    input_loss2 = losses2_.reshape(-1, 1)
    prob1 = np.zeros(targets_list.shape[0])
    prob2 = np.zeros(targets_list.shape[0])
    idx_chosen_sm = []
    idx_chosen_sm2 = []
    min_len = 1e10
    for j in range(num_class):
        indices = np.where(targets_list==j)[0]
        if len(indices) == 0:
            continue
        # REF2 : ils trichent ici
        bs_j = targets_list.shape[0] * (1. / num_class)
        pseudo_loss_vec_j = losses_[indices].tolist()
        pseudo_loss_vec_j2 = losses2_[indices].tolist()
        sorted_idx_j = np.argsort(pseudo_loss_vec_j)
        sorted_idx_j2 = np.argsort(pseudo_loss_vec_j2)
        ...
        idx_chosen_sm2.append(indices[sorted_idx_j2[:partition_j]])
        min_len = min(min_len, partition_j)
    idx_chosen_sm = np.concatenate(idx_chosen_sm)
    idx_chosen_sm2 = np.concatenate(idx_chosen_sm2)
    prob1[idx_chosen_sm] = 1
    prob2[idx_chosen_sm2] = 1
    
    # correction with Confidence Energy Data
    indexes = np.argsort((losses+losses2)/2)            
    indexes = indexes[-int(len(indexes) * rho * 2 / args.num_epochs):]
    targets_list[indexes] =  outputs_list.argmax(axis=-1)[indexes]

    return prob1, prob2, all_loss, targets_list.astype(int)
...
class NegEntropyConfidence(object):
    def __call__(self, outputs):
        outputs = outputs.clamp(min=1e-12)
        probs = torch.softmax(outputs, dim=-1)
        return torch.sum(probs.log() * (1-probs), dim=-1) / torch.sum(1-probs,dim=-1)
...
optimizer1 = FastAdaBelief([{'params': dualnet.net1.parameters()},
                           {'params': dualnet.net2.parameters()}] , lr=args.lr)

#dualnet = torch.load(f"./{args.dataset}_{args.noise_type}best.pth2.tar")
#dualnet.cuda()
...
confidence_penalty = NegEntropyConfidence()
...
for epoch in tqdm(range(args.num_epochs + 1)):
    ...
    if epoch < warm_up:
        ...
        eval_loader, noise_or_not = loader.run('eval_train')
        rho = args.rho_start + (args.rho_end - args.rho_start) * linear_rampup2(epoch, args.warmup_ep)
        prob1, prob2, all_loss[0], noisy_labels = eval_train(epoch, dualnet.net1, dualnet.net2, all_loss[0], rho, args.num_class)
        pred1 = (prob1 > args.p_threshold)
        #if epoch > args.num_epochs - args.start_expand:
        loader.save_label(noisy_labels)
