...
class ResNet(nn.Module):
    ...
    def forward(self, x, train=False,use_ph=False, contrastive=False):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        ...
        if train:
            ...
            if use_ph:
                ...
                if contrastive:
                    return out_linear, out_linear_debias, F.normalize(out, dim=1)
            ...
            if contrastive:
                if use_ph:
                    ...
                    return out_linear, F.normalize(out, dim=1),
            else:
                if use_ph:
                    out_linear_debias = self.pseudo_linear(out)
