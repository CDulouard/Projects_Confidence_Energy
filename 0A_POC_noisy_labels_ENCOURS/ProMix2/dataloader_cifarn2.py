...
class cifarn_dataset(Dataset):
    def __init__(self,  dataset,  noise_type, noise_path, root_dir, transform, mode, transform_s=None, is_human=True, noise_file='',
                 pred=[], probability=[],probability2=[] ,log='', print_show=False, r =0.2 , noise_mode = 'cifarn', dico=None):
        ...
        if self.mode == 'test':
            ...
            if os.path.exists(noise_file):
                ...
                print('good ones {:.3f} % '.format((1-(self.noise_or_not.sum())/len(self.noise_or_not))*100))
            else:    #inject noise   
        ...
        if self.dico is not None:
            for element in ["train_data", "probability", "probability2", "test_data", "test_label", "train_labels"]:
                if element in self.dico:
                   setattr(self, element, self.dico[str(element)])
                else:
                   if getattr(self, element) is not None:
                       self.dico[str(element)] = getattr(self, element)
            for element in ["probability", "probability2", "noise_label"]:
                if element in dico and dico[str(element)] is not None:
                    setattr(self, element, self.dico[str(element)])

        else:
            self.dico = {}
            for element in ["train_data", "train_labels", "probability", "probability2", "test_data", "test_label", "noise_label"]:
                if element not in self.dico and getattr(self, element) is not None:
                    self.dico[str(element)] = getattr(self, element)
        print('good two {:.3f} %'.format(100*(np.array(self.noise_label)==np.array(self.train_labels)).sum()/len(self.train_labels)))
...
class cifarn_dataloader():
    ...
    def run(self, mode, pred=[], prob=[],prob2=[]):
        if mode == 'warmup':
            ...
            self.dico = all_dataset.get_dico()
            ...
            self.dico = labeled_dataset.get_dico()
            ...
            self.dico = test_dataset.get_dico()
            ...
            self.dico = eval_dataset.get_dico()

            return eval_loader, eval_dataset.noise_or_not
        
    def save_label(self, noise_data):

        self.dico['noise_label'] = noise_data.tolist()
        json.dump(noise_data.tolist(), open(self.noise_file,"w"))
