...
parser.add_argument('--noise_file', default=None, type=str,help='noise file')
...
def train(epoch, net, net2, optimizer, labeled_trainloader, pi1, pi2, pi1_unrel, pi2_unrel):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, true_labels, index) in enumerate(labeled_trainloader):
        ...
        if args.penalty:
            # penalties
            penalty = -torch.nanmean(conf_penalty(outputs_x)) -torch.nanmean(conf_penalty(outputs_a))
        else:
            penalty = 0
...
def warmup(epoch, net, net2, optimizer, dataloader):
    ...
    for batch_idx, (inputs_w, inputs_s, labels, _) in enumerate(dataloader):
        ...
        if args.penalty:
            penalty = -torch.nanmean(conf_penalty(outputs)) - torch.nanmean(conf_penalty(outputs2))
        else:
            penalty = 0
...
def eval_train(epoch, model, model2, all_loss, rho, num_class):
    ...
    for j in range(num_class):
        ...
        pseudo_loss_vec_j = losses[indices].tolist()
        pseudo_loss_vec_j2 = losses2[indices].tolist()
...
class NegEntropy(object):
    def __call__(self, outputs):
        ...
        return torch.nanmean(probs.log() * probs, dim=1)
...
noise_file =args.noise_file if args.noise_file is not None else '%s/noise_file/%s_%s.json' % (args.data_path,args.noise_type,time)
...
loader = dataloader.cifarn_dataloader(args.dataset, noise_type=args.noise_type, noise_path=args.noise_path,
                                      is_human=args.is_human, batch_size=args.batch_size, num_workers=31, \
                                      root_dir=args.data_path, log=stats_log,
                                      noise_file=noise_file, r = args.noise_rate , noise_mode = args.noise_mode)
...
while True:
    ...
    for epoch in tqdm(range(args.num_epochs + 1)):
        ...
        acc = test(epoch, dualnet.net1, dualnet.net2)
        if acc > best_acc_:
            best_acc_ = acc
            print('best_acc',best_acc_)
        ...
        if early_stopping > 10:
            break
    rho = 0.5
    prob1, prob2, all_loss[0], noisy_labels = eval_train(200, dualnet.net1, dualnet.net2, all_loss[0], rho, args.num_class)
    pred1 = (prob1 > args.p_threshold)
    loader.save_label(noisy_labels)
    eval_loader, noise_or_not = loader.run('eval_train')
    dualnet = create_model()
