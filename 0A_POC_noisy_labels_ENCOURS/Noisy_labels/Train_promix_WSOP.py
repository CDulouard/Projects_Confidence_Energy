...
from utils.optimizer import FastAdaBelief
from utils.losses import SupConLoss, overparametrization_loss
...
parser.add_argument('--rho_range', default='0.2,0.6', type=str, help='ratio of selecting clean labels (rho)')
parser.add_argument('--tau', default=0.99, type=float, help='high-confidence selection threshold')
...
parser.add_argument('--contrastive', action='store_true', default=False, help='use contrastive (for high noise ratio)')
parser.add_argument('--penalty', action='store_true', default=False, help='use NegEntropy (for higher confidence results)')

args = parser.parse_args()

# REF1 : ils trichent ici
...
# Training
def train(epoch, net, net2, optimizer, optimizer_SOP, labeled_trainloader, pi1, pi2, pi1_unrel, pi2_unrel):
    net.train()
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, true_labels, index) in enumerate(labeled_trainloader):
        ...
        outputs_x, outputs_x_ph, outputs_embedding = net(inputs_x,train=True,use_ph=True, contrastive=True)
        outputs_x2, outputs_x2_ph, _ = net(inputs_x2,train=True,use_ph=True, contrastive=True)
        outputs_a, outputs_a_ph, outputs2_embedding = net2(inputs_x,train=True,use_ph=True, contrastive=True)
        outputs_a2, outputs_a2_ph, _ = net2(inputs_x2,train=True,use_ph=True, contrastive=True)
        outputs_x_ori = outputs_x.clone().detach()
        outputs_a_ori = outputs_a.clone().detach()
        outputs_embedding_ori = outputs_embedding.clone().detach()
        outputs2_embedding_ori =  outputs2_embedding.clone().detach()
        ...
        with torch.no_grad():
            ...
            labels_ = pseudo_label_l[idx_chosen].max(dim=1)[1]
            labels_2 = pseudo_label_l2[idx_chosen_2].max(dim=1)[1]
            
        if args.contrastive:
            # contrastive Losses
            contrastive1 = ContrastiveLoss(outputs_embedding[idx_chosen].unsqueeze(1), labels_)
            contrastive2 = ContrastiveLoss(outputs2_embedding[idx_chosen_2].unsqueeze(1), labels_2)
        ...
        loss_mix = SOP1(logits_mix, targets=pseudo_label_c_mix).mean()
        x_fmix = fmix(X_w_c)
        ...
        loss_mix_ph = SOP1(logits_mix_ph, targets=pseudo_label_c_mix_ph).mean()
        ...
        loss_cr = SOP1(outputs_x2[idx_chosen], targets=pseudo_label_l[idx_chosen]).mean()
        loss_cr_ph = SOP1(outputs_x2_ph[idx_chosen], targets=pseudo_label_l[idx_chosen]).mean()
        
        # cross entropy loss for primary head and pseudo head
        loss_ce = SOP1(outputs_x[idx_chosen], targets=pseudo_label_l[idx_chosen]).mean()
        loss_ce_ph = SOP1(outputs_x_ph[idx_chosen], targets=pseudo_label_l[idx_chosen]).mean()
        ...
        targets_urel = ptx
        loss_unrel_ph = SOP1(outputs_x_unrel_ph[idx_unchosen], targets=targets_urel[idx_unchosen]).mean()\
                  + w * SOP1(outputs_x2_unrel_ph[idx_unchosen], targets=targets_urel[idx_unchosen]).mean()
        ...
        loss_mix2 = SOP2(logits_mix2, targets=pseudo_label_c_mix2).mean()
        x_fmix2 = fmix(X_w_c)
        ...
        loss_mix2_ph = SOP2(logits_mix2_ph, targets=pseudo_label_c_mix2).mean()
        ...
        # consistency loss for primary head and pseudo head
        loss_cr2 = SOP2(outputs_a2[idx_chosen_2], targets=pseudo_label_l2[idx_chosen_2]).mean()
        loss_cr2_ph = SOP2(outputs_a2_ph[idx_chosen_2], targets=pseudo_label_l2[idx_chosen_2]).mean()
        # cross entropy loss for primary head and pseudo head
        loss_ce2 = SOP2(outputs_a[idx_chosen_2], targets=pseudo_label_l2[idx_chosen_2]).mean()
        loss_ce2_ph = SOP2(outputs_a_ph[idx_chosen_2], targets=pseudo_label_l2[idx_chosen_2]).mean()
        ...
        targets_urel2 = ptx2
        loss_unrel2_ph = SOP2(outputs_a_unrel_ph[idx_unchosen_2], targets=targets_urel2[idx_unchosen_2]).mean()\
                  + w * SOP2(outputs_a2_unrel_ph[idx_unchosen_2], targets=targets_urel2[idx_unchosen_2]).mean()
        ...
        if args.penalty:
            # penalties
            penalty = conf_penalty(outputs_x) + conf_penalty(outputs_a)
        else:
            penalty = 0

        # total loss
        loss = loss_net1 + loss_net2 + loss_net1_ph + loss_net2_ph + contrastive + penalty 
        L = loss + penalty
...
def warmup(epoch, net, net2, optimizer, optimizer_SOP, dataloader):
    net.train()
    net2.train()
    num_iter = (len(dataloader.dataset) // dataloader.batch_size) + 1
    for batch_idx, (inputs_w, inputs_s, labels, indexes) in enumerate(dataloader):
        inputs_w, inputs_s, labels = inputs_w.cuda(), inputs_s.cuda(), labels.cuda()
        optimizer.zero_grad()
        optimizer_SOP.zero_grad()
        
        outputs, outputs_embedding = net(inputs_w, contrastive=True)
        outputs2, outputs_embedding2 = net2(inputs_w, contrastive=True)
        l_ce = SOP1(indexes, outputs, labels)
        l_ce2 = SOP1(indexes, outputs2, labels)

        if args.contrastive:
            contrastive1 = ContrastiveLoss(outputs_embedding.unsqueeze(1))
            contrastive2 = ContrastiveLoss(outputs_embedding2.unsqueeze(1))
            contrastive = contrastive1 + contrastive2
        else:
            contrastive = 0
        ...
        loss = l_ce + l_ce2
        L = loss + contrastive +  penalty
        L.backward()
        optimizer.step()
        optimizer_SOP.step()

        if batch_idx % 100 == 0:
            print('%s:%s | Epoch [%3d/%3d] Iter[%3d/%3d]\t CE-loss: %.4f  All-loss: %.4f'
                         % (
                         args.dataset, args.noise_type, epoch, args.num_epochs, batch_idx + 1, num_iter, loss.item(), L.item()))
...
def eval_train(model, all_loss, rho, num_class):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, index) in enumerate(eval_loader):
            ...
            loss = CE(outputs, targets) - conf_penalty(outputs) 
...
optimizer = FastAdaBelief([{'params': dualnet.net1.parameters()}], lr=args.lr)
...
SOP1 = overparametrization_loss(num_examp=50000, num_classes=args.num_class, ratio_consistency=0.9, ratio_balance=0.1)
SOP2 = overparametrization_loss(num_examp=50000, num_classes=args.num_class, ratio_consistency=0.9, ratio_balance=0.1)
trainable_params = [{'params': [p for p in dualnet.net1.parameters() if  getattr(p, 'requires_grad', False)]}]
reparam_params = [{'params': SOP1.u, 'lr': 1, 'weight_decay': 0}, 
                  {'params': SOP1.v, 'lr': 10, 'weight_decay': 0}]
optimizer = optim.SGD(trainable_params, lr=1, momentum=0, weight_decay=0)
...
for epoch in range(args.num_epochs + 1):
    ...
    if epoch < warm_up:
        ...
        print('Warmup Net1')
        warmup(epoch, dualnet.net1, dualnet.net2, optimizer, optimizer_overparametrization, warmup_trainloader)
    if True:
        # REF4 : ils trichent ici
        rho = args.rho_start + (args.rho_end - args.rho_start) * linear_rampup2(epoch, args.warmup_ep)
        ...
        pi1,pi2,pi1_unrel,pi2_unrel = train(epoch,dualnet.net1, dualnet.net2, optimizer, optimizer_overparametrization, total_trainloader,pi1,pi2,pi1_unrel,pi2_unrel) 
